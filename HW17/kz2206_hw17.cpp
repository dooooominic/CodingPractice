...
template <class T>
void RBT<T>::singleCR(RBTNode<T> *&point) { //Left Left
    /*I am following https://www.geeksforgeeks.org/introduction-to-red-black-tree/ for rotation help*/
    ...
    //grandparent->left = nullptr; Might have subtrees and what not 
    ...
    point = parent;
    if (grandparent == root){ //update for root just in case
        root = point;
    }
}

template <class T>
void RBT<T>::singleCCR(RBTNode<T> *&point) { //Right Right 
    RBTNode<T> *grandparent = point;
...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        //1. No need to worry if parent is black. 
        //2. Parent is red
        while(curr_node!= nullptr && getColor(curr_node->parent)==RED){ 
            ...
            //if Uncle is black, rotate. If red, color both parent and uncle black. 
                if(getColor(uncle)==RED){ //grandparent with two red parent case! Rule 2
                    parent->color = BLACK;
                    uncle->color = BLACK;
                    grandparent->color = RED;
                    curr_node = grandparent; //send upwards to fix
                }
                else{ //uncle is black. two rotation cases
                    if(curr_node == parent->left){ //left left 
                        
                        parent->color = BLACK;
                        grandparent->color = RED;
                        singleCR(grandparent);
                    }
                    else if(curr_node == parent->right){ //left right
                ...
                if(getColor(uncle)==RED){ //grandparent with two red parent case! Rule 2
                    ...
                    if(curr_node == parent->right){ //right right
                        
                        parent->color = BLACK;
                        grandparent->color = RED;
                        singleCCR(grandparent);
                    }
                    else if(curr_node == parent->left){ //right left
    ...
    //always color root black at the end to finish the fix
    root->color = BLACK;
